...
import { CreditCard, Edit3, Check, X, TrendingDown } from 'lucide-react';
import { 
  mockTransactions, 
  mockAccounts, 
...
const Spending = () => {
  ...
  const [editableTransactions, setEditableTransactions] = useState(mockTransactions);
  ...
  const handleEditToggle = () => {
    ...
    if (isEditing) {
      ...
      console.log('Saving transaction changes:', editableTransactions);
    }
  };
  ...
  const getFilteredTransactions = () => {
    if (activeTab === 'all') {
      return editableTransactions.filter(t => t.amount < 0);
    }
    return editableTransactions.filter(t => t.amount < 0 && t.accountId === activeTab);
  };

  const uniqueAccounts = [...new Set(mockTransactions.map(t => t.accountId))];
                        ...
                        <td className="py-3 px-4 text-right">
                          {isEditing ? (
                            <Input
                              ...
                              onChange={(e) => updateTransaction(transaction.id, 'amount', -Math.abs(parseFloat(e.target.value) || 0))}
